...
public class SysUserController extends BaseController{
    ...
    {
        startPage();
        List<SysUserDTO> list = userService.selectUserList(user);
    ...
    public AjaxResult listselect(SysUserDTO user)
    {
    ...
    @GetMapping("/export")
    public AjaxResult export(SysUserDTO user)
    {
        List<SysUserDTO> list = userService.selectUserList(user);
    ...
    public GetInfoAjaxResult getInfo(@PathVariable(value = "userId", required = false) Long userId)
    {
        GetInfoAjaxResult ajax = GetInfoAjaxResult.success();
        List<SysRoleDTO> roles = roleService.selectRoleAll();
        ajax.setRoles(SysUserDTO.isAdmin(userId) ? roles : roles.stream().filter(r -> !r.isAdmin()).collect(Collectors.toList()));
        //TODO 去掉了岗位
        ajax.setPosts(null);
    ...
    @Log(title = "用户管理", businessType = BusinessType.UPDATE)
    @PutMapping("/resetPwd")
    public AjaxResult resetPwd(@RequestBody SysUserDTO user)
    {
        userService.checkUserAllowed(user);
        user.setPassword(SecurityUtils.encryptPassword(user.getPassword()));
        user.setUpdateBy(SecurityUtils.getUsername());
    ...
    public AjaxResult changeStatus(@RequestBody SysUserDTO user)
    {
        userService.checkUserAllowed(user);
        user.setUpdateBy(SecurityUtils.getUsername());
